...
PERMISSIONS = {
    ...
    "view_banquestionmark": False,
    # If the moderator doesn't fit the ['view_banstaff''] (previous) permission, show him just a "?" instead. Otherwise, it will be "Mod" or "Admin".
    ...
    "managestaff": "ADMIN",  # Manage staff
    "promotestaff": "ADMIN",  # Promote/demote staff
    "editusers": "ADMIN",  # Edit any users' login information
    "change_password": "JANITOR",  # Change user's own password
    "deletestaff": "ADMIN",  # Delete staff
...
def compare_groups(current_group: str, required_group: str) -> bool:
    return get_group_number(current_group.upper()) >= get_group_number(required_group.upper())
...
def has_permissions(group: str, action: str) -> bool:
    ...
    return compare_groups(group.upper(), PERMISSIONS[action])
